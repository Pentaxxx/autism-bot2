...
# Путь к базе данных (можно переопределить через переменную окружения DB_PATH)
DB_PATH = os.getenv("DB_PATH", "duel_stats.db")

# Создаём папку для БД, если её нет (для путей с папками)
...
def update_user_info(user_id: int, username: str):
    try:
        ...
        logging.error(f"Ошибка обновления информации пользователя {user_id}: {e}")
...
def get_user_id_by_username(username: str):
    try:
        ...
        logging.error(f"Ошибка поиска пользователя {username}: {e}")
        return None
...
def update_stats(user_id: int, win: bool = None, draw: bool = False):
    try:
        ...
        if row:
            ...
            if draw:
                ...
                if win:
                    ...
                    losses += 1
                total += 1
            cur.execute('UPDATE users SET wins = ?, losses = ?, total_duels = ? WHERE user_id = ?',
                        (wins, losses, total, user_id))
        else:
            if draw:
        ...
        logging.error(f"Ошибка обновления статистики для {user_id}: {e}")
...
async def handle_answer_button(callback: types.CallbackQuery):
    ...
    # Защита от отсутствия lock (для старых дуэлей)
    if 'lock' not in duel:
