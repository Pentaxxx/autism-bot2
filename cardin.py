...
def update_stats(user_id: int, win: bool = None, draw: bool = False):
    """
    win: True - победа, False - поражение, None - ничья
    draw: True - ничья (увеличиваем только total_duels)
    """
    try:
        conn = sqlite3.connect('duel_stats.db')
        cur = conn.cursor()
...
async def start_duel(challenger_id: int, opponent_id: int):
    ...
    duels[challenger_id] = {
        ...
        'lock': asyncio.Lock()   # явно создаём блокировку
    }
...
# ========== ОБРАБОТКА КНОПОК ОТВЕТОВ (исправлена с защитой lock) ==========
...
# ========== ЗАВЕРШЕНИЕ ДУЭЛИ (исправлена с защитой) ==========
